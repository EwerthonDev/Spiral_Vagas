      ...
      <header-vagas :usuarioLogado="usuarioLogado.name"></header-vagas>
        ...
        <div class="bg-gray-800 pt-6">
            <div class="container mx-auto p-4" style="max-width: 1000px;">
                ...
                <form class=" mt-8" v-on:submit.prevent="enviarVaga(vaga.id)">
                    <div v-if="vaga">
                        <label class="text-gray-100 font-bold text-xl"><b class="text-red-600">* </b>Nome da Vaga</label>
                        <input class="bg-gray-300 rounded w-full p-3" type="text" name="nome_vaga" value="" v-model="vaga.nome_vaga" />
                        
                        <div class="grid grid-cols-2 gap-4 mt-3">
                            <div class="col-span-1">
                                <label class="text-gray-100 font-bold text-xl"><b class="text-red-600">* </b>País</label><br>
                                <select class="bg-gray-300 w-full rounded p-3" name="pais" v-model="vaga.pais">
                                    <option value="Brasil">Brasil</option>
                                </select>
                            </div>
                            <div class="col-span-1">
                                <label class="text-gray-100 font-bold text-xl"><b class="text-red-600">* </b>Cidade</label><br>
                                <input class="bg-gray-300 w-full rounded p-3" type="text" name="cidade" v-model="vaga.cidade"/>
                            </div>
                            <div class="col-span-1">
                                <label class="text-gray-100 font-bold text-xl"><b class="text-red-600">* </b>Estado</label><br>
                                <input class="bg-gray-300 w-full rounded p-3" type="text" name="estado" v-model="vaga.estado" />
                            </div>
                            <div class="col-span-1 mb-3">
                                <label class="text-gray-100 font-bold text-xl"><b class="text-red-600">* </b>Departamento</label><br>
                                <select class="bg-gray-300 w-full rounded p-3" name="departamento" v-model="vaga.departamento">
                        ...
                        <select class="bg-gray-300 w-full rounded p-3 mb-3" name="tipo_emprego" v-model="vaga.tipo_emprego">
                            <option>CLT</option>
...
export default {
    ...
    methods: {
        enviarVaga: async function(vagaId)
        {
            axios.post(`/api/vagas/update/${vagaId}`, this.vaga).then(response => {
    ...
    mounted() {
        axios.get('/api/vagas/resgatarVaga').then(response => {
            this.vaga = response.data.vaga
        });
